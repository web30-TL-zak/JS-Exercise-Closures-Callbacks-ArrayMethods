...
function isItAnApple(strings) {
  let newArr = strings.map(item => item === 'apple' ? true : false)
...
function stringSmash(strings) {
  return strings.reduce((acc, cv) => acc + cv)
}
...
function getFullNames(runners) {
  let runnersArr = runners.map(person => `${person.last_name}, ${person.first_name}`)
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let runnersByShirtSize = runners.filter(person => {
...
function tallyUpDonations(runners) {
  let totalDonations = runners.map(person => person.donation)
  if(totalDonations.length === 0){
    return 0;
  } else {
    return totalDonations.reduce((acc, cv) => acc + cv)
  }
}
 ...
 *  -counter1 is being assigned the invokation of counterMaker() which is a function counter() returning count++.
 *  -counter2 is a function that just increments a global variable called count upon invoking the func
 * 
 * 2. Which of the two uses a closure? How can you tell?
 *  -both of these counters use closure, because both are reaching outwards from their scope to grab a variable
 * 
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better? 
 *  -it all depends on if you will need access to that count variable anywhere else in this code. In this case the variable would best be 
 * suited in the function body, because multiple counters having access to a single count variable may not be the best idea
 *
*/
